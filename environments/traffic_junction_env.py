...
class TrafficJunctionEnv(gym.Env):
    ...
    def __init__(self,):
        ...
        self.curr_start = 250 # start making harder after this many epochs [0]
        self.curr_end = 1250 # when to make the game hardest [0]
        ...
        if self.vocab_type == 'bool':
            ...
            self.obs_dim = 1 + 1 + self.vocab_size*(2*self.vision+1)*(2*self.vision+1)
        else:
            # r_i, (x,y), vocab = [road class + car]
            self.vocab_size = 1 + 1

            self.obs_dim = 1 + 1 + 2 + self.vocab_size*(2*self.vision+1)*(2*self.vision+1)
