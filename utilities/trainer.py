...
class PGOfflineTrainer(object):
    ...
    def run_batch(self, t):
        ...
        batch = Transition(*zip(*batch))
        stat = self.train_batch(t, batch, stats)
        return stat
...
class PGOnlineTrainer(object):
    ...
    def train_online(self, stat, i):
        ...
        for t in range(self.args.max_steps):
            ...
            if self.args.replay:
                ...
                if i > self.args.replay_warmup and len(self.replay_buffer.buffer) >= self.args.batch_size:
                    self.replay_process(stat)
            else:
                self.online_process(stat, trans)
            if self.args.target:
                if t%self.args.target_update_freq == self.args.target_update_freq-1:
                    self.behaviour_net.update_target()
            if done_:
    ...
    def online_process(self, stat, trans):
    ...
    def run_batch(self, t):
        ...
        average_num_steps = 0
        while num_episodes < self.args.epoch_size:
            mean_reward, num_steps = self.train_online(stats, t)
            average_mean_reward += mean_reward
            average_num_steps += num_steps
            num_episodes += 1
        stats['mean_reward'] = average_mean_reward / self.args.epoch_size
